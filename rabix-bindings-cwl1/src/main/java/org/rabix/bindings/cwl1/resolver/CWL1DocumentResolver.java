...
public class CWL1DocumentResolver {
  ...
  private static Map<String, Map<String, CWL1DocumentResolverReference>> referenceCache = new HashMap<>();
  private static Map<String, LinkedHashSet<CWL1DocumentResolverReplacement>> replacements = new HashMap<>();
  ...
  public static String resolve(String appUrl) throws BindingException {
    ...
    for (CWL1DocumentResolverReplacement replacement : getReplacements(appUrl)) {
  ...
  private static JsonNode traverse(String appUrl, JsonNode root, File file, JsonNode parentNode, JsonNode currentNode, boolean addReplacement) throws BindingException {
    ...
    if (isReference || isJsonPointer) {
      ...
      CWL1DocumentResolverReference reference = getReferenceCache(appUrl).get(referencePath);
      if (reference != null) {
        if (reference.isResolving()) {
          throw new BindingException("Circular dependency detected!");
        }
      } else {
        reference = new CWL1DocumentResolverReference();
      ...
      if (addReplacement) {
        getReplacements(appUrl).add(new CWL1DocumentResolverReplacement(parentNode, currentNode, referencePath));
      }
  ...
  private static void replaceObjectItem(String appUrl, JsonNode root, CWL1DocumentResolverReplacement replacement) throws BindingException {
    ...
    CWL1DocumentResolverReference reference = getReferenceCache(appUrl).get(replacement.getNormalizedReferencePath());
    if (reference != null) {
  ...
  private static void replaceArrayItem(String appUrl, JsonNode root, CWL1DocumentResolverReplacement replacement) throws BindingException {
    JsonNode parent = replacement.getParentNode() == null ? root : replacement.getParentNode();
  ...
  private synchronized static Set<CWL1DocumentResolverReplacement> getReplacements(String url) {
    LinkedHashSet<CWL1DocumentResolverReplacement> replacementsPerUrl = replacements.get(url);
    if (replacementsPerUrl == null) {
      replacementsPerUrl = new LinkedHashSet<CWL1DocumentResolverReplacement>();
  ...
  private synchronized static Map<String, CWL1DocumentResolverReference> getReferenceCache(String url) {
    Map<String, CWL1DocumentResolverReference> referenceCachePerUrl = referenceCache.get(url);
    if (referenceCachePerUrl == null) {
      referenceCachePerUrl = new HashMap<String, CWL1DocumentResolverReference>();
