...
public class CWL1Translator implements ProtocolTranslator {

  @Override
  public DAGNode translateToDAG(Job job) throws BindingException {
    CWL1Job cwl1Job = CWL1JobHelper.getCWL1Job(job);
    DAGNode dagNode = processBatchInfo(cwl1Job, transformToGeneric(cwl1Job.getId(), cwl1Job));
  ...
  private DAGNode processBatchInfo(CWL1Job job, DAGNode node) {
    ...
    if (batch != null) {
      ...
      if (batch instanceof List<?>) {
        ...
        scatterList.add(CWL1SchemaHelper.normalizeId((String) batch));
      } else {
  ...
  private DAGNode transformToGeneric(String globalJobId, CWL1Job job) throws BindingException {
    List<DAGLinkPort> inputPorts = new ArrayList<>();
    
    for (ApplicationPort port : job.getApp().getInputs()) {
      DAGLinkPort linkPort = new DAGLinkPort(CWL1SchemaHelper.normalizeId(port.getId()), job.getId(), LinkPortType.INPUT, LinkMerge.merge_nested, port.getScatter() != null ? port.getScatter() : false);
      inputPorts.add(linkPort);
    }
    List<DAGLinkPort> outputPorts = new ArrayList<>();
    for (ApplicationPort port : job.getApp().getOutputs()) {
      DAGLinkPort linkPort = new DAGLinkPort(CWL1SchemaHelper.normalizeId(port.getId()), job.getId(), LinkPortType.OUTPUT, LinkMerge.merge_nested, false);
    ...
    for (CWL1DataLink dataLink : workflow.getDataLinks()) {
