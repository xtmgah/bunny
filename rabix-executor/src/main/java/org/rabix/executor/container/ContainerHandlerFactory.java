...
public class ContainerHandlerFactory {
  ...
  public static ContainerHandler create(Job job, Requirement requirement, DockerClientLockDecorator dockerClient, Configuration configuration) throws ContainerException {
    if (requirement instanceof DockerContainerRequirement) {
      return new DockerContainerHandler(job, (DockerContainerRequirement) requirement, configuration, dockerClient);
    }
