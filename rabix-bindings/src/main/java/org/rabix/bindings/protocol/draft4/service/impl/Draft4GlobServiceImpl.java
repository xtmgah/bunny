...
public class Draft4GlobServiceImpl implements Draft4GlobService {

  private final Logger logger = LoggerFactory.getLogger(Draft4GlobServiceImpl.class);

  /**
   * Find all files that match GLOB inside the working directory 
   */
  @SuppressWarnings("unchecked")
  public Set<File> glob(Draft4Job job, File workingDir, Object glob) throws Draft4GlobException {
    Preconditions.checkNotNull(job);
    Preconditions.checkNotNull(workingDir);
    
    try {
      if (Draft4ExpressionResolver.isExpressionObject(glob)) {
        glob = Draft4ExpressionResolver.resolve(glob, job, null);
      }
    } catch (Draft4ExpressionException e) {
      logger.error("Failed to evaluate glob " + glob, e);
      throw new Draft4GlobException("Failed to evaluate glob " + glob, e);
    }
    if (glob == null) {
    ...
    for (String singleGlob : globs) {
      ...
      try {
        ...
        throw new Draft4GlobException("Failed to traverse through working directory", e);
      }
    }
    return files;
  }
