...
public class Draft4Translator implements ProtocolTranslator {
  ...
  private DAGNode transformToGeneric(String globalJobId, Draft4Job job) throws BindingException {
    ...
    for (ApplicationPort port : job.getApp().getInputs()) {
      DAGLinkPort linkPort = new DAGLinkPort(Draft4SchemaHelper.normalizeId(port.getId()), job.getId(), LinkPortType.INPUT, LinkMerge.merge_nested, port.getScatter() != null ? port.getScatter() : false);
      inputPorts.add(linkPort);
    }
    List<DAGLinkPort> outputPorts = new ArrayList<>();
    for (ApplicationPort port : job.getApp().getOutputs()) {
      DAGLinkPort linkPort = new DAGLinkPort(Draft4SchemaHelper.normalizeId(port.getId()), job.getId(), LinkPortType.OUTPUT, LinkMerge.merge_nested, false);
